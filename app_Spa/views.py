...
def ver_resenas(request):
    resenas = Reseñas.objects.all().select_related('usuario', 'pedido')
    return render(request, 'resenas/ver_resenas.html', {'resenas': resenas})
...
def actualizar_resena(request, id_resena):
    resena = get_object_or_404(Reseñas, id_reseña=id_resena)
    usuarios = Usuarios.objects.all()
    pedidos = Pedidos.objects.all()

    if request.method == 'POST':
        resena.usuario_id = request.POST.get('usuario')
        resena.pedido_id = request.POST.get('pedido')
        resena.titulo = request.POST.get('titulo')
        resena.comentario = request.POST.get('comentario')
        resena.calificacion = request.POST.get('calificacion')
        resena.save()
        return redirect('ver_resenas')

    return render(request, 'resenas/actualizar_resena.html', {
        'resena': resena,
        'usuarios': usuarios,
        'pedidos': pedidos
    })
...
def borrar_resena(request, id_resena):
    ...
    if request.method == 'POST':
        resena.delete()
        return redirect('ver_resenas')
    return render(request, 'resenas/borrar_resena.html', {'resena': resena})
